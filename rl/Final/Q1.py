...
class WindyGrid:
    ...
    def get_sarsa_vf_and_policy(
        ...
        for episode_num in range(episodes):
            ...
            while state in q :
                a_t0 = self.epsilon_greedy_action(state, q, epsilon)
                state_t1, reward_t1 = sample_func(state, a_t0)
                if state_t1 in q :
                    a_t1 = self.epsilon_greedy_action(state_t1, q, epsilon)
                    q[state][a_t0] = q[state][a_t0] + step_size*(reward_t1 + 1*q[state_t1][a_t1] - q[state][a_t0])
                else :
                    q[state][a_t0] = q[state][a_t0] + step_size*(reward_t1 - q[state][a_t0])
                state = state_t1
    ...
    def get_q_learning_vf_and_policy(
        ...
        for episode_num in range(episodes):
            ...
            while state in q :
                ...
                state_t1, reward_t1 = sample_func(state, a_t0)
                if state_t1 in q :
                    q[state][a_t0] = q[state][a_t0] + step_size*(reward_t1 + 1*max(q[state_t1].values()) - q[state][a_t0])
                else :
                    q[state][a_t0] = q[state][a_t0] + step_size*(reward_t1 - q[state][a_t0])
                state = state_t1
            '''
